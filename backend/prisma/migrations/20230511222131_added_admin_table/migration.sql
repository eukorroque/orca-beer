...
CREATE TABLE `Usuario` (
    ...
    `ultimaValidacaoTelefone` DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP(3),
    `avaliacao` INTEGER NOT NULL DEFAULT 0,
    `cashback` DOUBLE NULL DEFAULT 0,
    `alcance` INTEGER NULL,
    `vezesIndicou` INTEGER NULL DEFAULT 0,
    `qtdPedidosRestantes` INTEGER NULL DEFAULT 1,
...
-- AddForeignKey
ALTER TABLE `ProdutoTemp` ADD CONSTRAINT `ProdutoTemp_unidadeId_fkey` FOREIGN KEY (`unidadeId`) REFERENCES `unidadeProduto`(`id`) ON DELETE RESTRICT ON UPDATE CASCADE;
